...
import { AlertTriangle, Calendar, CheckCircle2, Edit3, Check } from 'lucide-react';
import styles from './FinanzasDashboard.module.css';

const STORAGE_KEY_DIA = 'safelink_monotributo_dia';
const STORAGE_KEY_PAGADO = 'safelink_monotributo_pagado_mes';

function getMesActualKey(): string {
  const hoy = new Date();
...
export function MonotributoCard() {
  const [diaVenc, setDiaVenc] = useState<number>(() => {
    const stored = localStorage.getItem(STORAGE_KEY_DIA);
    return stored ? parseInt(stored, 10) : 20;
  });

  const [pagado, setPagado] = useState<boolean>(() => {
    const stored = localStorage.getItem(STORAGE_KEY_PAGADO);
    return stored === getMesActualKey();
  });
  ...
  const semaforo = pagado ? 'verde' : getSemaforo(diasRestantes);

  const handleGuardarDia = () => {
    const n = parseInt(inputVal, 10);
    if (!isNaN(n) && n >= 1 && n <= 31) {
      setDiaVenc(n);
      localStorage.setItem(STORAGE_KEY_DIA, String(n));
    }
    setEditando(false);
  };
  ...
  const handleTogglePagado = () => {
    if (pagado) {
      localStorage.removeItem(STORAGE_KEY_PAGADO);
      setPagado(false);
    } else {
      localStorage.setItem(STORAGE_KEY_PAGADO, getMesActualKey());
      setPagado(true);
    }
  };

  const labelEstado = pagado
    ? '✓ Al día'
    : {
        verde: '✓ Al día',
        amarillo: '⚠ Próximo',
  ...
  return (
    <div className={`${styles.monotributoCard} ${styles[semaforo]}`}>
      <div className={styles.monotributoTopRow}>
      ...
      <div className={`${styles.monotributoBig} ${styles[semaforo]}`}>
        {pagado ? 'OK' : diasRestantes <= 0 ? 'HOY' : `${diasRestantes}d`}
      </div>

      <p className={styles.monotributoSub}>
        {pagado
          ? 'Pago abonado para este mes'
          : diasRestantes <= 0
      ...
      <div className={styles.monotributoActions}>
        <button
          className={`${styles.pagadoBtn} ${pagado ? styles.pagadoBtnActive : ''}`}
          onClick={handleTogglePagado}
        >
          {pagado ? <Check size={14} /> : <CheckCircle2 size={14} />}
          {pagado ? 'Pagado (Al día)' : 'Pagado'}
        </button>
      </div>
    </div>
  );
...
export function MonotributoBanner() {
  ...
  if (pagado || (semaforo !== 'naranja' && semaforo !== 'rojo')) return null;
